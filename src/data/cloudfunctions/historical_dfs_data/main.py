...
import api.historical_dfs_data as dfs
import api.utils as utils

def update_historical_dfs_data(request):
    ...
    dg_df = dfs.get_dfs_events(**primary_params)
    ...
    if all(dg_events.isin(bq_events)):
        ...
        missing_events_df_with_dfs = pd.concat([missing_events_df.assign(site='draftkings'), 
                                                missing_events_df.assign(site='fanduel')], 
                                               ignore_index=True)

        api_params = missing_events_df_with_dfs.to_dict(orient="records")

        event_list = list(map(lambda x: dfs.get_dfs_data(**{**primary_params, **x}), api_params))
        event_df = pd.concat(event_list)

        # get table schema
        table = client.get_table(f'{projectid}.{tablename}.points')
        ...
        utils.upload_to_bq(
                table_id = f'{projectid}.{tablename}.points', 
